...
if prompt := st.chat_input("Example: What is Llama Stack?"):
    ...
    with st.chat_message("assistant"):
        ...
        # Add temperature and top_p if using top_p strategy
        if temperature > 0.0:
            api_params["temperature"] = temperature
            api_params["top_p"] = top_p
        
        # Add repetition_penalty if > 1.0
        if repetition_penalty > 1.0:
            api_params["frequency_penalty"] = repetition_penalty - 1.0

        response = llama_stack_api.client.chat.completions.create(**api_params)
